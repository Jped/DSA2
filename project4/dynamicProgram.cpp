...
int dynamicHelperFunction(std::vector<std::vector<int>> * dpMatrix, std::string string1, std::string string2, std::string mergedString) { 
	...
	if (string2Length !=0 && mergedString.back() == string2.back() && (*dpMatrix)[string1Length][string2Length-1] == 0){
		status = dynamicHelperFunction(dpMatrix,string1, string2.substr(0,string2Length-1), mergedString.substr(0, mergedString.length()-1));
		(*dpMatrix)[string1Length][string2Length-1] = status;
		if (status == 1)
			return status;

	}
	if (string1Length != 0 && mergedString.back() == string1.back() && (*dpMatrix)[string1Length-1][string2Length] == 0){
		status = dynamicHelperFunction(dpMatrix, string1.substr(0,string1Length-1), string2, mergedString.substr(0, mergedString.length()-1));
		(*dpMatrix)[string1Length-1][string2Length] = status;
	}
	return status;
} 
...
void isInterpolation(std::string string1, std::string string2, std::string mergedString, std::string outFileName) {
	...
	if (string1.length() + string2.length() == mergedString.length())
		dynamicHelperFunction(&dpMatrix, string1, string2, mergedString);
