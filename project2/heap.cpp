...
int heap::deleteMin(std::string *p_id, int *p_key, void *pp_data) {
	...
	heap::data[1] = heap::data[current_size]; 
	current_size--;
...
int heap::remove(const std::string &id, int *p_key, void *pp_data){
	if (!heap::mapping->contains(id))
		return 1;
	// first make the item have the lowest possible value
	heap::setKey(id, INT8_MIN);
	// then delete min
	heap::deleteMin();
	return 0;
...
void heap::percolateUp(int pos_cur){
	...
	for(;pos_cur>1 && item.key <= heap::data[pos_cur/2].key; pos_cur/2){
